...
export const ObjectsList3D: React.FC<ObjectsList3DProps> = ({
  ...
  const handleDoubleClick = (obj: EditorObject3D) => {
    setEditingId(obj.id);
  ...
  const handleDrop = (e: React.DragEvent, toIndex: number) => {
    e.preventDefault();
    if (draggedIndex !== null && draggedIndex !== toIndex) onReorder(draggedIndex, toIndex);
      ...
      <div className="panel-content">
        {objects.length === 0 ? (
          <div className="p-4 text-center text-muted-foreground text-sm">
            Aucun objet 3D. Utilisez Objet → Ajouter 3D.
          </div>
        ) : (
          objects.map((obj, index) => (
            ...
            >
              ...
              {editingId === obj.id ? (
                ...
                <span className="flex-1 truncate text-sm cursor-text" onDoubleClick={() => handleDoubleClick(obj)}>
                  {obj.name}
                </span>
              )}
              <Button
